...
from reviewcopies.models import Timeslot, Course, User
    ...
    def validate(self, attrs):
        ...
        for student_slot in student_slots:
            if student_slot.slot.schedule.date == slot.schedule.date:
                student_begin_time_truncated = student_slot.slot.begin_time.replace(year=1, month=1, day=1, second=0, microsecond=0).astimezone(timezone.get_current_timezone())
                slot_begin_time_truncated = slot.begin_time.replace(year=1, month=1, day=1, second=0, microsecond=0).astimezone(timezone.get_current_timezone())

                student_end_time_truncated = student_slot.slot.end_time.replace(year=1, month=1, day=1, second=0, microsecond=0).astimezone(timezone.get_current_timezone())
                slot_end_time_truncated = slot.end_time.replace(year=1, month=1, day=1, second=0, microsecond=0).astimezone(timezone.get_current_timezone())
                ...
                if (student_begin_time_truncated < slot_end_time_truncated and student_end_time_truncated > slot_begin_time_truncated)\
                    or (student_begin_time_truncated < slot_begin_time_truncated and student_end_time_truncated > student_end_time_truncated)\
                    or student_begin_time_truncated == slot_begin_time_truncated or student_end_time_truncated == slot_end_time_truncated :
                    raise serializers.ValidationError(f"Vous êtes deja inscrit au cours de {student_slot.course.name} durant ces heures")
