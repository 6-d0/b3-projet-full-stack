...
from reviewcopies.serializers.sessions import (CreateSessionSerializer,
...
class DeleteCourseFromSession(APIView):
    def delete(self, request, *args, **kwargs):
        ...
        if serializer.is_valid:
            return Response(
                {"detail": "Cours supprimé avec succès.", "session": serializer.data},
    ...
    def patch(self, request, *args, **kwargs):
        ...
        if serializer.is_valid:
            return Response(
                {"detail": "Cours ajouté avec succès.", "session": serializer.data},
                status=status.HTTP_201_CREATED,
            )
        else:
